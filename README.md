...
| `squad-redteam` | skill | Cross-model red-team / second-opinion review — dispatches the same artifact or question to several `drill-*` reviewers in parallel (user picks the panel via a multi-select question, defaulting to the single strongest drill per provider) and has the orchestrator cross-analyze their independent verdicts into one consolidated report. Same parallel-dispatch-and-consolidate idea as [`@alexmkx/opencode-multi-delegate`](https://github.com/AlexMKX/opencode-multi-delegate), reusing the squad's own drills instead of a separate delegate config |
